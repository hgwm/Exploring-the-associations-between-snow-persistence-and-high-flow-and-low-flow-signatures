# plotter_sites_signatures_climatic_indices.R plots overall signature/climatic data for all available watersheds in the study
...
sitesPlot <- ggplot(data = northAmerica) +
  ...
  labs(fill = NULL) +
  plot_annotation(theme = theme(plot.title = element_text(
    hjust = 0.5,
    size = 22,
    margin = margin(b = 15, unit = "pt")
  )))
ggsave(
  filename = paste(plotDir, sitesPlotFigure, filenameTag, ".png", sep = ""),
  plot = sitesPlot,
  device = "png",
  dpi = 320
)
...
for (i in 1:catLen) {
  ...
  if (currCat %in% keys(plotTitlesHash)) {
    ...
    longTermPlotFileName <- create_sig_filenames_string(
      ...
      suffixString = paste(filenameTag, ".png", sep = "")
    )
    ggsave(
...
allCatPlotsName <- paste(plotDir, attributeGeoPlotFigures, filenameTag, ".png", sep = "")
